<?php
/**
 * File containing the bcstaticcache class file
 *
 * @copyright Copyright (C) 1999 - 2013 eZ Systems AS., 1999 - 2014 Brookins Consutling and 2007 Damien POBEL. All rights reserved.
 * @license http://www.gnu.org/licenses/gpl-2.0.txt GNU General Public License v2
 * @version 2013.5 (modified)
 * @package bcgeneratestaticcache
 */

/**
 * The BCStaticCache class manages the static cache system.
 *
 ...
 * @package kernel
 */
class BCStaticCache implements ezpStaticCache
{
    /**
     * User-Agent string
     */
    ...
    /**
     * Getter method for {@link BCStaticCache::$hostName}
     *
    ...
    /**
     * Getter method for {@link BCStaticCache::$staticStorageDir}
     *
    ...
    /**
     * Getter method for {@link BCStaticCache::$maxCacheDepth}
     *
    ...
    /**
     * Getter method for {@link BCStaticCache::$cachedSiteAccesses}
     *
    ...
    /**
     * Getter method for {@link BCStaticCache::$cachedURLArray}
     *
    ...
    /**
     * Getter method for {@link BCStaticCache::$alwaysUpdate}
     *
     * These URLs are configured with AlwaysUpdateArray in staticcache.ini.
     *
     * @see BCStaticCache::generateAlwaysUpdatedCache()
                ...
                foreach ( $cacheFiles as $file )
                {
                    if ( !$skipUnlink || !file_exists( $file ) )
                    {
                        ...
                        else
                        {
                            ...
                            if ( $content === false )
                            {
                                if ( eZHTTPTool::getDataByURL( $fileName, true, BCStaticCache::USER_AGENT ) )
                                    $content = eZHTTPTool::getDataByURL( $fileName, false, BCStaticCache::USER_AGENT );
                            }
                            if ( $content === false )
                            {
                            ...
                            {
                                BCStaticCache::storeCachedFile( $file, $content );
                            }
                        }
                    }
                }
            ...
            switch( $action ) {
                case 'store':
                    ...
                    else
                    {
                        ...
                        {
                            if ( eZHTTPTool::getDataByURL( $source, true, BCStaticCache::USER_AGENT ) )
                                $fileContentCache[$source] = eZHTTPTool::getDataByURL( $source, false, BCStaticCache::USER_AGENT );
                            else
                        ...
                        {
                            BCStaticCache::storeCachedFile( $destination, $fileContentCache[$source] );
                            $doneDestList[$destination] = 1;
                        }
                    }
                    break;
